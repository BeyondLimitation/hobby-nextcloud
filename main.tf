...
module "moniter-vpc" {
  ...
  public_subnets  = var.mon_public_subnets
...
# Certificate Uploaded in ACM
data "aws_acm_certificate" "issued4vpn-server" {
  domain = "server"
}

data "aws_acm_certificate" "issued4vpn-client" {
  domain = "vpn.lee-heechan.world"
}

# Create Client VPN Endpoint
resource "aws_ec2_client_vpn_endpoint" "monitering_vpn_endpoint" {
  description            = "Monitering-vpn"
  server_certificate_arn = data.aws_acm_certificate.issued4vpn-server.arn
  client_cidr_block      = var.mon_private_subnets[1]

  security_group_ids = [module.vpn-endpoint-sg.security_group_id]
  vpc_id = module.moniter-vpc.vpc_id

  authentication_options {
    type                       = "certificate-authentication"
    root_certificate_chain_arn = data.aws_acm_certificate.issued4vpn-client.arn
  }
...
resource "aws_ec2_client_vpn_network_association" "vpn_moniter_association"{
  client_vpn_endpoint_id = aws_ec2_client_vpn_endpoint.monitering_vpn_endpoint.id
  subnet_id              = module.moniter-vpc.public_subnets[0]
}
resource "aws_ec2_client_vpn_authorization_rule" "vpn_moniter_rule" {
  client_vpn_endpoint_id = aws_ec2_client_vpn_endpoint.monitering_vpn_endpoint.id
  target_network_cidr    = var.mon_vpc_cidr
  authorize_all_groups   = true
}

# Create Security Group for Endpoint
module "vpn-endpoint-sg" {
  source  = "terraform-aws-modules/security-group/aws"
  version = "4.4.0"

  name        = "vpn-sg"
  description = "Security group for VPN.Allow ssh, http/s traffics inbound and outbound"
  vpc_id      = module.moniter-vpc.vpc_id
